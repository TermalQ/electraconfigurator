                  ...
                  echo '<div class="gadget__image py-2 mb-2 text-center rounded">';
                    if (!empty($item['image'])) {
                        ...
                        echo '<div class="thumbnail">';
                            echo '<img loading="lazy" decoding="async" fetchPriority="low" class="img-fluid" src="'. htmlspecialchars($versionedImagePath) .'" alt="' . $item['name'] .' - '. $item['desc'] .'">';
                        echo '</div>';
                    }
                  echo '</div>';
            ...
            echo '<div class="col-12 col-md-4">';
                ...
                if (!empty($item['colors']['values'])) {
                    ...
                    foreach ($item['colors']['values'] as $key => $value) {
                        ...
                        echo '<span class="' . $checkedClass . '" data-color="' . $value['title'] . '" data-item-key="' . $itemKey . '" data-image="' . htmlspecialchars($value['image']) . '" data-image-src="' . htmlspecialchars($versionedImagePath) . '" style="background:'. $value['color'] .'" title="'. $value['title'] .'"></span>';
                    }
                    echo '</div>';
                    echo '</div>';
                }
                ...
                echo '<div class="d-flex justify-content-between align-items-center">';
                    ...
                    if ($item['active']) {
                        ...
                        class="btn ' . ($item['show_price'] ? 'btn-warning' : 'btn-danger') . (!empty($item['shipping']['class']) ? ' ' . $item['shipping']['class'] : '') . ' addToCart"
                        tabindex="0"
                        title="'. $button .'"
                        aria-label="'. $button .'"
                        ...
                        // Jeśli ukrywamy cenę => 'do wyceny'
                        if ($item['show_price']) {
                            echo svg_code('cart.svg');
                        } else {
                            echo svg_code('pricing.svg');
                        }
                        echo '</button>';
                    } else {
