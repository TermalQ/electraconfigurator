...
if ($configuratorData['action'] === 'addToCart') {
    if (empty($_SESSION['cart']['products']['paka']['addToCart'])) {
        $_SESSION['cart']['products']['paka']['addToCart'] = true;
        $_SESSION['cart']['products']['paka']['timestamp'] = time();

        $status = 'added';  // oznacza pierwsze dodanie

    } else {
        $_SESSION['cart']['products']['paka']['timestamp'] = time();

        $status = 'updated';  // oznacza aktualizację
    }
...
echo json_encode([
    'success' => false,
    'message' => 'Nieprawidłowa akcja podczas dodawania paki do koszyka.'
]);
exit;
