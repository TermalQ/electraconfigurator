    ...
    foreach ($orderData as $key => $value) {
        if ($key === 'main_product') {
            ...
            if (!empty($value['flag'])) {
                $flag = $value['flag'];
                $mainProductHtml .= '<tr>';
                $mainProductHtml .= '<td colspan="5" style="padding: 10px; background-color: #f2f2f2; font-weight: bold;">' . htmlspecialchars($flag['title']) . ': ' . htmlspecialchars($flag['position']) . ' ' . htmlspecialchars($flag['align']) . '</td>';
                $mainProductHtml .= '</tr>';
                $mainProductHtml .= '<tr>';
