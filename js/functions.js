...
// ========================================
// Pokaż Errors => kolejkowanie => tablica
// ========================================
const notificationQueue = [];
let isNotificationVisible = false;

function showError(message, typeMessage = 'error-message') {
    notificationQueue.push({ message, typeMessage });
...
function showNextNotification() {
    ...
    const { message, typeMessage } = notificationQueue.shift();
    ...
    // Po 2 sekundach fade out i usunięcie
    setTimeout(() => {
        errorDiv.style.opacity = '0';
        setTimeout(() => {
            errorDiv.remove();
            // Po usunięciu wyświetl następne powiadomienie
            showNextNotification();
        }, 500);
    }, 2000);
}
...
async function updateCartSummary() {
    ...
    document.querySelectorAll('.shoppingCart .Cost').forEach(item => {
        ...
        // Pomijaj pozycje z ceną ujemną (czyli do wyceny)
        if (price < 0 || (promoPrice !== null && promoPrice < 0)) {
            // Ukryj cenę
            if (priceElement) priceElement.classList.add('d-none')
            if (promoElement) promoElement.classList.add('d-none')
        }
    ...
    try {
        ...
        if (cartValueElement) {
            // Jeśli cena <= 0, pokazujemy status "do wyceny" z (treść przycisku 'save-config') lub promo lub normalną cenę
            if (totalRegularPrice <= 0) {
                const saveConfigBtn = document.getElementById('save-config');
                const statusText = saveConfigBtn ? saveConfigBtn.getAttribute('data-status') : '---';
                cartValueElement.innerHTML = statusText;
            } else if (hasPromo) {
