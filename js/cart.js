document.addEventListener('DOMContentLoaded', function() {
    ...
    const addPackMessage    = saveConfigButton.dataset.add;
    const updatePackMessage = saveConfigButton.dataset.update;
    ...
    function generatePackBlockInCartHTML(paka) {
        ...
        const html = `
                ...
                <div class="d-flex align-items-center justify-content-between gap-2">
                    ...
                    <div class="Cost">
                        ${
                            // Jeśli cena >= 0, pokazujemy cenę i walutę, w przeciwnym razie status "do uzgodnienia"
                            typeof paka.price === 'number' && paka.price >= 0
                                ? paka.promo
                                    // Jeśli cena promocyjna jest dostępna, pokazujemy ją
                                    ? `<del class="Price">${formatNumberWithSpaces(paka.price)}</del>&nbsp;
                                       <span id="mainProductPromo" class="Promo">${formatNumberWithSpaces(paka.promo)}</span>&nbsp;
                                       <span class="Currency">${currency}</span>`
                                    : `<span id="mainProductPrice" class="Price">${formatNumberWithSpaces(paka.price)}</span>&nbsp;
                                       <span class="Currency">${currency}</span>`
                ...
                <div id="cartOptions" class="cartBoxOptions my-2">
                    ...
                    <div class="boxColor d-flex justify-content-between align-items-center">
                        ...
                        <div class="Cost">
                            ${
                                // Jeśli cena >= 0, pokazujemy cenę i walutę, w przeciwnym razie status "do uzgodnienia => &nbsp;"
                                typeof paka.color.price === 'number' && paka.color.price >= 0
                                    ? `<span id="mainProductColorPrice" class="Price">${formatNumberWithSpaces(paka.color.price)}</span>&nbsp;
                                       <span class="Currency">${currency}</span>`
                                    : `<span id="mainProductColorPrice" class="Price Status">${'&nbsp;'}</span>`
                            }
                        </div>
                    </div>
                </div>
        ...
        function addPackToCart() {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    // Wyświetl odpowiedni komunikat na podstawie statusu zwróconego z PHP
                    if (data.status === 'added') {
                        showError(addPackMessage, 'success-message');
                    } else if (data.status === 'updated') {
                        showError(updatePackMessage, 'success-message');
                    }
    ...
    function getFlagData(flagCheckboxId, inputCountryId) {
        ...
        // ==========================================
        // Znajdź aktualny przycisk z klasą .current
        const currentButton = document.querySelector('.flag.current');

        // Pobierz dodatkowe dane z przycisku, jeśli istnieje
        const buttonPosition = currentButton?.dataset.position || '';
        const buttonAlign = currentButton?.dataset.alignName || '';


        return {
            FlagTitle: title ? encodeHTML(title) : '',
            FlagPosition: buttonPosition ?? false,
            FlagAlign: buttonAlign ?? false,
    ...
    function getFlagOptions() {
        ...
        fetch('include/update-flag.php', {
            ...
            body: JSON.stringify({
                ...
                Position: flagData.FlagPosition,
                Align: flagData.FlagAlign,
        ...
        if (flagCheckbox.checked) {
            ...
            if (flagData.UserCountry) {
                ...
                flagOptionElement.innerHTML = `
                    <div class="Title">${encodeHTML(flagData.FlagTitle)}: ${encodeHTML(flagData.FlagPosition)}</div>
                    <span class="Align">${encodeHTML(flagData.FlagAlign)}</span>
                    ...
                    <div class="d-flex align-items-center justify-content-between gap-2">
                        ...
                        <div class="Cost">
                            ${
                                // Jeśli cena >= 0, pokazujemy cenę i walutę, w przeciwnym razie status "do uzgodnienia => &nbsp;"
                                typeof flagData.FlagPrice === 'number' && flagData.FlagPrice >= 0
                ...
                itemElement.innerHTML = `
                    <div class="row d-flex align-items-center">
                        ...
                        <div class="col-3 Cost">
                            ${
                                // Jeśli cena >= 0, pokazujemy cenę i walutę, w przeciwnym razie status "do uzgodnienia"
                                typeof itemTotal === 'number' && itemTotal >= 0
                                    ? `<span class="Price">${formatNumberWithSpaces(itemTotal)}</span> <span class="Currency">${currency}</span>`
