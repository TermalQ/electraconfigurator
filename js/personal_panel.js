document.addEventListener("DOMContentLoaded", function () {
    ...
    function updateVisibility() {
        ...
        // ============================================
        // Ukryj wszystkie <rect> gdy nie ma chechbox'a
        // ============================================
        if (!flagChecked) {
            document.querySelectorAll('rect[id^="preview_flag"]').forEach(rect => {
                rect.style.visibility = 'hidden';
            });
        }

        if (flagChecked) {
            // Resetuj do domyślnego buttona i recta
            const defaultRect = document.getElementById('preview_flagTopPanel_left');
            const defaultButton = document.getElementById('buttonFlag_flagTopPanel_left');
            const defaultRadio = document.getElementById('flagLabel_flagTopPanel');
        
            // Ukryj wszystkie inne recty
            const allRects = document.querySelectorAll('rect[id^="preview_flag"]');
            allRects.forEach(rect => rect.style.visibility = 'hidden');
        
            // Usuń klasy z innych buttonów
            const allButtons = document.querySelectorAll('.flag');
            allButtons.forEach(btn => btn.classList.remove('current'));
        
            const allLabels = document.querySelectorAll('.accordion-label');
            allLabels.forEach(label => label.classList.remove('current'));
        
            // Pokaż domyślny rect
            if (defaultRect) {
                defaultRect.style.visibility = 'visible';
            }
        
            // Ustaw domyślny button jako aktywny
            if (defaultButton) {
                defaultButton.classList.add('current');
                const label = defaultButton.closest('.accordion-label');
                if (label) {
                    label.classList.add('current');
                }
            }
        
            // Zaznacz domyślne radio
            if (defaultRadio) {
                defaultRadio.checked = true;
                defaultRadio.setAttribute('aria-checked', 'true');
                defaultRadio.setAttribute('aria-pressed', 'true');
                defaultRadio.setAttribute('aria-selected', 'true');
            }
        }
        // ================================
